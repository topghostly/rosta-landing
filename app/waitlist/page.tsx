...
export default function WaitlistPage() {
  return (
    <main className="min-h-screen w-full bg-[#faf9f5] text-[#141413]">
      <div className="mx-auto flex min-h-screen w-full max-w-[1440px] flex-col px-6">
        <header className="flex items-center justify-between py-6 md:py-8">
          <Link
            href="/"
            className="flex items-center gap-2 font-serif text-[20px] tracking-tight text-[#141413]"
          >
            <svg
        ...
        C337.8,176.2,337.8,176.2,280.8,175.7z"
            ...
            <span className="font-serif text-[32px] md:text-[38px] tracking-tight text-[#141413]">
              Rosta
            </span>
          </Link>
        </header>

        <div className="flex flex-1 items-start justify-center py-12 md:py-20">
          <div className="grid w-full max-w-[1080px] grid-cols-1 gap-12 md:grid-cols-2 md:gap-20">
            <div className="flex flex-col md:sticky md:top-12 md:self-start">
              <div className="inline-flex w-fit items-center gap-1 justify-center rounded-[4px] border border-background/[0.08] bg-background/[0.04] px-2.5 py-1 text-background">
                <span className="text-background/55">
                  <Ear size={14} />
                </span>
                <span className="font-sans text-[12px] font-medium tracking-tight text-background/55">
                  EARLY ACCESS
                </span>
              </div>

              <h1 className="mt-6 font-serif text-[40px] leading-[1.04] tracking-tight text-[#141413] sm:text-[52px] md:text-[60px]">
                Join the Rosta Waitlist.
              </h1>

              <p className="mt-6 max-w-[440px] font-sans text-[16px] leading-[1.55] text-[#5e5d59] md:text-[17px]">
                Founding agency pricing, early access, and a direct line to the
                team building Rosta. Six fields. Less than a minute.
              </p>

              <ul className="mt-10 flex flex-col gap-3 font-sans text-[14px] text-[#3a3937]">
                <ListItem>Locked-in early pricing in naira</ListItem>
                <ListItem>First access when we ship</ListItem>
                <ListItem>Direct feedback channel with the team</ListItem>
              </ul>
            </div>
        ...
        <footer className="border-t border-[#141413]/8 py-6">
          <p className="font-sans text-[14px] text-muted">
            © {new Date().getFullYear()} Rosta. Built for African influencer
            agencies.
          </p>
        </footer>
      </div>
...
function ListItem({ children }: { children: React.ReactNode }) {
  return (
    <li className="flex items-start gap-2.5">
      <span
        aria-hidden="true"
        className="mt-[7px] inline-block h-1.5 w-1.5 rounded-full bg-[#141413]"
      />
      <span>{children}</span>
    </li>
  );
